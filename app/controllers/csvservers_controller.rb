...
class CsvserversController < ApplicationController
  ...
  def root
    ...
    User.includes(:company).find_each do |user|
      temp_string = "#{user.first_name},#{user.last_name},#{user.email},#{user.opt_in},#{user.created_at},#{user.company.name},#{user.company.venue},#{user.company.created_at}\n"
      response.stream.write temp_string
    end
